...
images.map((image) => {
  ...
  const imageElement = `
      <div class="filter-image" data-filter="${image.split('-')[0]}">
        <span><img src="images/${image}" alt="${image.split('-')[0]} image" /></span>
      </div>  
  `;
...
function activateFilter() {
  ...
  Array.from(imageElements).filter((imageElement) => {
    ...
    }, 100); //시간 지연 함수(promise): 첫번째 파라미터 = callback function, 두번째 파라미터 = 시간(ms)밀리초
  });
}

// 함수 정리 예
// function ClickEvent(els, el, eventFn){
...
const showLightBox = (e) => {
  ...
  const selectedImage = target.children[0].children[0].getAttribute('src'); // 클릭한 이미지 경로 보여줌

  const categoryName = target.getAttribute('data-filter');

  const lightBoxImage = document.querySelector('.light-box-image img');
  const categortElement = document.querySelector('.title p');

  // getAttribute(): 파라미터 속성 값 가져오기
  // setAttribute(a, b): a: 속성 이름, b: 변경할 속성 값
  // a.textContent = b: a 요소에 b 텍스트 입력
  lightBoxImage.setAttribute('src', selectedImage); // src 요소에 이미지 경로 입력
  categortElement.textContent = categoryName;
